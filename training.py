...
class Training:
    def __init__(self, data, vocab_size=100, embedding_dim=32, hidden_size=64, output_dim=5, final_embeddings = None, device: torch.device = torch.device('cpu')):
        self.data = data
        ...
        self.model = TransformerModel(self.vocab_size, self.embedding_dim, self.hidden_size, num_heads=2, num_encoder_layers=2, output_dim=self.output_dim, pre_trained_weights = final_embeddings, dropout = 0.5, device = self.device).to(self.device)
